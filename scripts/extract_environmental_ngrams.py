...
def get_token_length(row):
    ...
    # subtract 2 to remove <start> <end> token counts
    return token_encoding['length'].item() - 2
...
def filter_phrase(df, max_phrase_length=150, max_token_length=20):
    ...
    df = df[~df['phrase str'].str.contains('|'.join(['\t', '\n', '\r', '\b']))]
...
def count_positive_negative_phrase(df, ngram):
    ...
    print(f'Positive phrae: {len(positive_phrase_counts)}')
    ...
    print(f'Phrase count: {len(df_phrase_counts)}')
...
if __name__ == '__main__':
    ...
    ap.add_argument('--data_csv_path', type=str, help='Path to csv with extracted data from msgpack, data_path is omitted if this is passed in')
